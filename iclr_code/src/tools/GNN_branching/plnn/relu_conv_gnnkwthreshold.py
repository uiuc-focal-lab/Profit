...
def relu_gnn(net, domain,  x, ball_eps, bounded, eps=1e-4, pgd_threshold = 1, sparsest_layer=0, branching_threshold = 0.2, kwbd_threshold=10, split_decision=None, decision_bound=None, linear=False, model_path=None, dump_trace = None):
    ...
    bounds_indices = [0]+pre_relu_indices + [len(net.net)]
    layers = {}; 
    layers['fixed_layers'] =[copy.deepcopy(i).cuda() for i in net.net[:-1]]
    layers['prop_layers'] = [copy.deepcopy(net.net[-1]).cuda()]
    ...
    print('gnn_decision: ', gnn_decision)
    ...
    while global_ub - global_lb > eps:
        ...
        print(gnn_improvement, branching_threshold)

        if gnn_improvement < branching_threshold:
            # use kw decision
            kw_decision, icp_score = choose_node_conv(orig_lbs, orig_ubs, mask, net.net, pre_relu_indices, icp_score, random_order, sparsest_layer)
        ...
        if dom_lb >= decision_bound:
            pruned_masks.append(dom_updated_mask)
            print("Pruned domain: ", len(domains))
        
        if dom_lb1 >= decision_bound:
            pruned_masks.append(dom_updated_mask1)
            print("Pruned domain: ", len(domains))
        ...
        if dom_lb < decision_bound:
            ...
            print("Added domain: ", len(domains))
            prune_counter += 1
    ...
    print("length: ", len(pruned_masks))

    # for domain in pruned_domains:
        
    return global_lb, global_ub, global_ub_point, nb_visited_states, pruned_masks


def relu_gnn_with_masks(net, domain,  x, ball_eps, bounded, eps=1e-4, pgd_threshold = 1, sparsest_layer=0, branching_threshold = 0.2, kwbd_threshold=10, split_decision=None, decision_bound=None, linear=False, model_path=None, dump_trace = None, pruned_masks=None):
    '''
    Uses branch and bound algorithm to evaluate the global minimum
    of a given neural network.
    `net`           : Neural Network class, defining the `get_upper_bound` and
    ...
    if pruned_masks == None:
        print('gnn_decision: ', gnn_decision)
        candidate_domain = ReLUDomain(updated_mask, global_lb, global_ub, lower_bounds_all, upper_bounds_all, gnn_decision=gnn_decision)
        domains = [candidate_domain]
    else:
        # update the domains with global bounds
        domains = []
        for pruned_mask in pruned_masks:
            gnn_decision = graph.decision(lower_bounds_graph, upper_bounds_graph, dual_vars,global_ub_point, primals, layers, pruned_mask)

            mask_temp = [i.clone() for i in pruned_mask]
            
            dom_ub, dom_lb, dom_ub_point, dom_dual_vars, dom_dual_vars_other, dom_primals, dom_updated_mask, dom_lb_all, dom_ub_all = net.get_lower_bound(mask_temp, lower_bounds_all, upper_bounds_all, gnn_decision, 0)
        
            candidate_domain = ReLUDomain(dom_updated_mask, lb=dom_lb, ub= dom_ub, lb_all= dom_lb_all, up_all = dom_ub_all, gnn_decision = gnn_decision)
...
def kw_split(net, candidate_domain):
    ...
    decision = choose_dim(orig_lbs, orig_ubs, mask, net.net)
